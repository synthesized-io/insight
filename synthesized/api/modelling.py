...
def predictive_modelling_score(data: pd.DataFrame, y_label: str, x_labels: List[str], model: str, sample_size: Optional[int] = None):
    ...
    return _predictive_modelling_score(data=data, model=model, y_label=y_label, x_labels=x_labels, sample_size=sample_size)
...
def predictive_modelling_comparison(data: pd.DataFrame, synth_data: pd.DataFrame,
    ...
    return _predictive_modelling_comparison(
        data=data, synth_data=synth_data, model=model, y_label=y_label, x_labels=x_labels, sample_size=sample_size
    )
