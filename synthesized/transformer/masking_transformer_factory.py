...
from .child import (SwappingTransformer, RoundingTransformer, RandomTransformer,
...
class MaskingTransformerFactory:
    ...
    def create_random_transformer(self, masked_column, arg_val) -> RandomTransformer:
        str_length = 10
        if arg_val:
            str_length = int(arg_val)
        return RandomTransformer(name=masked_column, str_length=str_length)

    def create_partial_transformer(self, masked_column, arg_val) -> PartialTransformer:
    ...
    def create_rounding_transformer(self, masked_column, arg_val) -> RoundingTransformer:
        n_bins = 20
        if arg_val:
            n_bins = int(arg_val)
    ...
    def create_transformers(self, config: Dict[str, str]) -> DataFrameTransformer:
        ...
        for masked_column, masking_technique in config.items():
            ...
            if masking_technique.startswith('random'):
                transformers.append(self.create_random_transformer(masked_column, arg_val))
            elif masking_technique.startswith('partial_masking'):
                transformers.append(self.create_partial_transformer(masked_column, arg_val))
            elif masking_technique.startswith('rounding'):
                transformers.append(self.create_rounding_transformer(masked_column, arg_val))
