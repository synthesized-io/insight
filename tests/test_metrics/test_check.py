...
from insight import ColumnCheck
    ...
    df = pd.DataFrame(
        {
            ...
            "date_col": pd.to_datetime(
                18_000 + np.random.normal(500, 50, size=1000).astype(int), unit="D"
            ),
            ...
            "ordered_cat_col": pd.Categorical(
                np.random.choice(["b", "d", "c"], size=1000),
                categories=["b", "c", "d"],
                ordered=True,
            ),
        }
...
def test_column_check(df):
    ...
    ordinal_cols = set(
        ["ordered_cat_col", "date_col", "float_col", "int_col", "int_bool_col", "bool_col"]
    )
...
def test_check_ordinal():
    ...
    sr = pd.Series(
        pd.Categorical(
            np.random.choice(["t2", "t1", "t0"], size=100),
            categories=["t0", "t1", "t2"],
            ordered=True,
        )
    )
