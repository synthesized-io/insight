...
from insight.metrics import EarthMoversDistance, HellingerDistance
from insight.metrics.confidence_interval import (
    compute_binomial_interval,
...
def test_binomial_interval():
    conf_interval = compute_binomial_interval(
        pd.Series([1, 1]), pd.Series([0, 0]), confidence_level=0.99
    )
    assert conf_interval.level == 0.99
    assert conf_interval.limits[0] is not None and conf_interval.limits[1] is not None

    conf_interval = compute_binomial_interval(
        pd.Series([1, 0]), pd.Series([1, 0]), confidence_level=0.80
    )
