...
def verify_columns_types(df, categorical_cols, continuous_cols, affine_cols, ordinal_cols, binary_cols):
    check = ColumnCheck()
    ...
    pred_binary_cols = set()
    for col in df.columns:
        if check.binary(df[col]):
            pred_binary_cols.add(col)
    assert pred_binary_cols == binary_cols
...
def test_column_check(df):
    ...
    binary_cols = set(['bool_col', 'int_bool_col'])

    verify_columns_types(df.copy(), categorical_cols, continuous_cols, affine_cols, ordinal_cols, binary_cols)
    ...
    verify_columns_types(df_nan, categorical_cols, continuous_cols, affine_cols, ordinal_cols, binary_cols)
