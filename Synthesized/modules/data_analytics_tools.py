...
class DataPipeline():
	...
	def prepare_dataset_for_training(self):
		...
		for ids in unique_ids:
		    for month in months:
		        v = pd.DataFrame({}, index = range(0, spendings_clusters), columns = range(1,transactions_time_resolution+1))
		   ...
		   # e3 = e3.fillna(0)
		        v = v.fillna(0.)
		        fw = self.targetDataSet[(self.targetDataSet["seqId"] == ids) & (self.targetDataSet["date"].dt.month == month)]
	...
	def get_outlier_dataset(self):
		...
		outlier_dataset['seqId'] = np.concatenate([np.repeat(i,12) for i in [first_cluster_of_customers, second_cluster_of_customers]],axis = 0)
	...
	def get_aggregated_dataset_for_snapshot_generation(self, targetDataSet, multiplier, spendings_clusters):
		...
		for ids in unique_ids:
		    v = pd.DataFrame({}, columns = range(0, spendings_clusters * multiplier), index = range(1,transactions_time_resolution+1))
		    v = v.fillna(0.)
		    for month in months:
		        ...
		        if (fw.shape[0]!= 0) : 
		            for i in xrange(fw.shape[0]):
		                v.set_value(int(fw["date"][i].day), int(fw["category"][i]) *  multiplier + (month % multiplier) , fw["value"][i])
	...
	def compress_dataset(seld, dataSet, multiplier, clusters_number):
		...
		for dataSetInstance in dataSet:
		    a = pd.DataFrame(data=dataSetInstance, index=np.arange(1,33), columns=np.arange(1, clusters_number * multiplier + 1)) 
		    ...
		    for i in np.arange(1, a.shape[0] + 1):
		        for k in np.arange(1, b.shape[1] + 1):
		            b[k][i] = random.choice([a[(k-1) * multiplier + 1][i], a[(k-1)*multiplier + 2][i], a[(k-1) * multiplier + 3][i]])
		    newDataSet.append(b)
		return(newDataSet)
